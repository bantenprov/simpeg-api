...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
			//echo $key_name;
			//test($_SERVER);
			//test($this->input->server(array('SERVER_PROTOCOL', 'REQUEST_URI', 'SERVER_ADDR', 'HTTP_HOST', 'HTTP_CLIENT')));
			//test($this->input->server('SERVER_ADDR'));
			...
			$host = $this->input->server('HTTP_CLIENT');
			if(sha1($host) != $key){
				$this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => sprintf($this->lang->line('text_rest_invalid_domain'), $host)
            ], self::HTTP_FORBIDDEN);
            $this->is_valid_request = false;
				return FALSE;
			}
